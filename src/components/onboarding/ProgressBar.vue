<template>
  <div class="w-full px-2 py-4 sm:px-4">
    <p class="text-sm font-medium text-warm-grey mb-3 sm:mb-4" aria-live="polite">
    ...
    <div class="flex items-center justify-between gap-0" role="progressbar" :aria-valuenow="currentStep" aria-valuemin="1" aria-valuemax="6" :aria-label="`Paso ${currentStep} de 6`">
      <template v-for="(_, index) in 6" :key="index">
        <div
          class="w-8 h-8 sm:w-10 sm:h-10 rounded-full flex items-center justify-center text-sm font-medium transition-colors duration-300 flex-shrink-0"
        ...
        <span
          ...
          :class="index + 1 < currentStep ? 'bg-soft-terracota' : 'bg-warm-sand'"
          aria-hidden="true"
        />
      </template>
    </div>
...
function stepClass(step) {
  if (step < props.currentStep) return 'bg-soft-terracota text-off-white'
  if (step === props.currentStep) return 'bg-soft-terracota text-off-white ring-2 ring-soft-terracota ring-offset-2'
  return 'bg-warm-sand text-warm-grey'
}
</script>
